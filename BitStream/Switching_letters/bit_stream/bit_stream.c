...
int readChar(FILE* source_file,int char_index){
    ...
    if(read_char == -1){
        printf("Current character in postion %d is %i ,(EOF), which translates to -0b1 in binary\n",char_index,read_char);
        return read_char;
    ...
    printf("Current character in postion %d is %i, which translates to %s in binary\n",char_index,read_char,binary_char);
...
int main(int argc,char* argv[]){
    ...
    printf("What's the character index you'd like to change?\n");
    int char_index = getInt();

    int read_char = readChar(file,char_index);
    writeChar(read_char,char_index,file);
